...
//it reorgnize the unmuted role and the member and add the unmuted and remove the muted role
    let mutedRole = message.guild.roles.cache.get('866556969966764062'); // u put the muted role ID
    let verifiedRole = message.guild.roles.cache.get('791192730751205408'); // the member role ID
    if(mutedRole) {
